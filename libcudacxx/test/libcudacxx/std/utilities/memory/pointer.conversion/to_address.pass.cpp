...
struct STDP3
{
  TEST_FUNC constexpr explicit STDP3(int* p)
      : p_(p)
  {}
  int* p_;
};
...
template <>
struct std::pointer_traits<STDP3>
{
  TEST_FUNC static constexpr int* to_address(const STDP3& p)
  {
    return p.p_;
  }
};
...
struct MixedP3
{
  TEST_FUNC constexpr explicit MixedP3(int* p)
      : p_(p)
  {}
  int* p_;
};
...
template <>
struct std::pointer_traits<MixedP3>
{
  TEST_FUNC static constexpr int* to_address(const MixedP3& p)
  {
    return p.p_;
  }
};
...
template <>
struct cuda::std::pointer_traits<MixedP3>
{
  TEST_FUNC static constexpr int* to_address(const MixedP3&)
  {
    return nullptr;
  }
};
...
struct STDP4
{
  TEST_FUNC constexpr explicit STDP4(int* p)
      : p_(p)
  {}
  TEST_FUNC int* operator->() const; // should never be called
  int* p_;
...
template <>
struct std::pointer_traits<STDP4>
{
  TEST_FUNC static constexpr int* to_address(const STDP4& p)
  {
    return p.p_;
  }
};
...
{
  T* p_;
  TEST_FUNC constexpr STDFancyPtrB(T* p)
      : p_(p)
  {}
  TEST_FUNC T& operator*() const;
};
...
template <class T>
struct std::pointer_traits<P8::STDFancyPtrB<T>>
{
  TEST_FUNC static constexpr T* to_address(const P8::STDFancyPtrB<T>& p)
  {
    return p.p_;
  }
};
...
TEST_FUNC constexpr bool test()
{
  ...
  // Ensure we also consider std::pointer_traits
  STDP3 stdp3(&i);
  static_assert(noexcept(cuda::std::to_address(stdp3)));
  assert(cuda::std::to_address(stdp3) == &i);
  STDP4 stdp4(&i);
  static_assert(noexcept(cuda::std::to_address(stdp4)));
  assert(cuda::std::to_address(stdp4) == &i);

  // Ensure we prefer the ::std::pointer_traits overload
  MixedP3 mixedp3(&i);
  static_assert(noexcept(cuda::std::to_address(mixedp3)));
  assert(cuda::std::to_address(mixedp3) == &i);
  ...
  static_assert(cuda::std::is_same_v<decltype(cuda::std::to_address(cuda::std::declval<STDP6>())), int const*>);
  ...
  P8::STDFancyPtrB<Holder<Incomplete>> stdp8b = p8_nil;
  assert(cuda::std::to_address(stdp8b) == p8_nil);
  static_assert(cuda::std::is_same_v<decltype(cuda::std::to_address(stdp8b)), decltype(p8_nil)>);
